...
@StudyHeader(
        namespace="com.tystr",
        id="TYSTR_RELATIVE_VOLUME_LABEL",
        name="Tystr's Relative Volume Label",
        menu="Examples",
        desc="DESC_RELATIVE_VOLUME",
        overlay=true,
        requiresVolume=true,
        requiresBarUpdates=true)
public class RelativeVolumeLabel extends com.motivewave.platform.sdk.study.Study
{
    ...
    protected void calculateValues(DataContext ctx)
    {
        ...
        // ADFASDFASDFASDF
        int index = series.size()-10;
        long time = series.getStartTime(index);
        float price = series.getClose(index);
        debug("Adding label...");
        Coordinate coordinate = new Coordinate(time, price - 5);
        Label label = new Label(coordinate, "RV: " + series.getFloat(Values.REL_VOL));
        addFigure(Plot.PRICE, label);

        debug("Adding arrow at index: " + index);
        debug("time: " + time);
        debug("price: " + price);
        Marker arrow = new Marker(new Coordinate(time, price - 10), Enums.MarkerType.ARROW);
